...
export const getProductsAPI = (): Observable<Product[]> => {
  return httpClient.get(API.products)
    .pipe(map(result => result.response.map((p: any) => deserialize(Product, p))));
};

export const getProductRatingsAPI = (productId: string): Observable<ProductRating[]> => {
  return httpClient.get(API.productRatings, {params: {productId}})
...
export const productSearchAPI = (query: string = ""): Observable<Product[]> => {
  ...
  return httpClient.get(API.productSearch, {queryParams: {criteria: query}})
    .pipe(map(results => results.response.products.map((p: any) => deserialize(Product, p))),
      catchError(() => of([])));
};
